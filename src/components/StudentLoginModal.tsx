...
export const StudentLoginModal: React.FC<StudentLoginModalProps> = ({
  ...
  const findAvatarId = (student?: StudentProfile | null): string => {
    if (!student) return STUDENT_AVATARS[0]?.id ?? 'student_1';
    const direct = STUDENT_AVATARS.find((a) => a.emoji === student.avatarEmoji);
    if (direct) return direct.id;
    if (student.avatarEmoji === '👧🏽') return 'student_2';
    if (student.avatarEmoji === '👦🏻') return 'student_3';
    if (student.avatarEmoji === '👩🏾') return 'student_4';
    if (student.avatarEmoji === '🧒🏿') return 'student_6';
    if (student.avatarEmoji === '🎨') return 'student_8';
    return STUDENT_AVATARS[0]?.id ?? 'student_1';
  };

  const [selectedAvatarId, setSelectedAvatarId] = useState<string>(
    findAvatarId(currentStudent)
  );
            ...
            <form onSubmit={handleCreateOrUpdate} className="space-y-4">
                ...
                <div className="flex items-center gap-1.5 p-1 bg-slate-100 dark:bg-blue-950/70 rounded-2xl border border-slate-200 dark:border-blue-900">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSelectedAvatarId(findAvatarId(currentStudent));
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10.5px] font-extrabold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/80 px-2 py-0.5 rounded-full border border-blue-200/80 dark:border-blue-800/80">
                  ...
                  {STUDENT_AVATARS.map((avatar) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group p-2 sm:p-2.5 rounded-2xl border-2 flex flex-col items-center gap-1.5 transition-all duration-200 cursor-pointer relative overflow-hidden ${
                          isSelected
                            ? 'border-blue-600 dark:border-blue-400 bg-gradient-to-b from-blue-50/90 via-sky-50/40 to-white dark:from-blue-950/90 dark:via-blue-900/30 dark:to-[#132240] shadow-md shadow-blue-500/20 ring-3 ring-blue-500/40 -translate-y-0.5'
                            : 'border-slate-200/90 dark:border-blue-900/70 bg-white/90 dark:bg-[#132240] hover:border-blue-300 dark:hover:border-blue-600 hover:bg-blue-50/40 dark:hover:bg-[#182b4f] hover:-translate-y-0.5 hover:shadow-xs'
                        }`}
                        title={`${avatar.label} — ${avatar.role}`}
                      >
                        {/* Avatar Bubble */}
                        <div className="relative">
                          <div
                            className={`w-12 h-12 sm:w-13 sm:h-13 rounded-2xl bg-gradient-to-tr ${avatar.bgColor} p-0.5 shadow-md flex items-center justify-center border-2 border-white/80 dark:border-white/20 transition-transform duration-200 group-hover:scale-108`}
                          >
                            <span className="text-2xl sm:text-3xl select-none drop-shadow-md">
                              {avatar.emoji}
                            </span>
                          </div>

                          {/* Floating Role Accessory Badge */}
                          <div
                            className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-white dark:bg-slate-900 border-2 border-white dark:border-slate-800 shadow-xs flex items-center justify-center text-[10px] select-none"
                            title={avatar.role}
                          >
                            {avatar.badgeEmoji}
                          </div>
                        ...
                        <div className="w-full text-center min-w-0">
                          ...
                          <span className="text-[9px] font-bold text-slate-400 dark:text-slate-400 block truncate uppercase tracking-tight">
                            {avatar.tag}
                          </span>
                        </div>
                        ...
                        {isSelected && (
                          <div className="absolute top-1 right-1 w-4.5 h-4.5 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 text-white flex items-center justify-center shadow-xs animate-in zoom-in-50">
                ...
                {/* Selected Avatar Motto & Personality Card */}
                {selectedAvatar && (
                  <div className="mt-2.5 p-2.5 rounded-2xl bg-gradient-to-r from-blue-50 via-teal-50/50 to-indigo-50 dark:from-blue-950/60 dark:via-blue-900/40 dark:to-indigo-950/60 border border-blue-200/80 dark:border-blue-800/80 flex items-center gap-2.5 animate-in fade-in duration-200">
                    <div className={`w-9 h-9 rounded-xl bg-gradient-to-tr ${selectedAvatar.bgColor} flex items-center justify-center text-xl shadow-xs shrink-0 border border-white/80 dark:border-white/20`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className="text-[9.5px] font-extrabold uppercase px-1.5 py-0.2 rounded-md bg-blue-600/15 dark:bg-blue-400/20 text-blue-800 dark:text-blue-300">
                          {selectedAvatar.role}
                        </span>
                      </div>
